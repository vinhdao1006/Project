...
import { Search, UserPlus, ChevronLeft, ChevronRight, UserRound, Building2, Calendar, Check, X } from 'lucide-react';
                ...
                {Object.keys(statusStyles).map((status) => (
                  <button
                    ...
                    className={`flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 
                      ${filterStatus === status
                        ? `${statusStyles[status].bg} ${statusStyles[status].text} border border-${status === 'Available' ? 'bimec-green/30' : 'red-300/30'}`
                        : 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-50'}`}
                ...
                <tbody className="divide-y divide-gray-100">
                  {paginatedDoctors.map((doctor, idx) => (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-bimec-green to-bimec-heavy-green flex items-center justify-center text-white text-xs font-medium mr-3">
                            ...
                            {doctor.name.split(' ')[2] ? doctor.name.split(' ')[2][0] : ''}
                          </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className={`text-sm ${doctor.todayAppointments > 0 ? 'text-gray-900 font-medium' : 'text-gray-500'}`}>
                            {doctor.todayAppointments} appointment{doctor.todayAppointments !== 1 ? 's' : ''}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${statusStyles[doctor.status].bg} ${statusStyles[doctor.status].text}`}>
