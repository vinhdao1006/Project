...
function DoctorAppoinments() {
    ...
    const fetchAppointments = async () => {
      try {
        ...
        const formattedAppointments = response.data.map((appt) => ({
          patientName: appt.patientInfo
            ? `${appt.patientInfo.firstname} ${appt.patientInfo.lastname}`
            : "Unknown Patient",
          time: appt.appointmentTime,
          concern: appt.reason,
          rawData: appt, // Store full appointment data for navigation
        }));
  ...
  const filteredPatients = appointments.filter(
    (patient) =>
  ...
  const handlePatientClick = (patient) => {
    navigate("/doctor/patient-detail", { state: { patient: patient.rawData } });
  };
