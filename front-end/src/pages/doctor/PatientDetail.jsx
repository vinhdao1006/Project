    ...
    <div className="flex items-center gap-2 text-sm">
      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
        selectedDate === new Date().toISOString().split('T')[0]
          ? 'bg-bimec-light-green text-bimec-heavy-green' : 'bg-gray-100'
      }`}>
        {selectedDate === new Date().toISOString().split('T')[0] ? 'Today' : formatDate(selectedDate)}
      </span>
      <button onClick={() => changeDate(-1)} className="p-1.5 rounded-full bg-white border border-gray-200 hover:bg-bimec-light-green text-gray-500 hover:text-bimec-heavy-green">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="w-16 h-16 rounded-full bg-bimec-light-green flex items-center justify-center border-2 border-bimec-green">
              <span className="text-bimec-heavy-green text-xl font-bold">{getInitials(patient.patient)}</span>
            </div>
          ...
          <div>
            <h2 className="font-semibold text-bimec-black text-lg">{patient.patient}</h2>
...
const Vitals = () => {
  ...
  const vitalColors = [
    { bg: "bg-blue-50", text: "text-blue-700", icon: <HeartIcon className="h-5 w-5" /> },
    { bg: "bg-purple-50", text: "text-purple-700", icon: <ScaleIcon className="h-5 w-5" /> },
    { bg: "bg-amber-50", text: "text-amber-700", icon: <ClockIcon className="h-5 w-5" /> },
    { bg: "bg-rose-50", text: "text-rose-700", icon: <FireIcon className="h-5 w-5" /> },
    { bg: "bg-emerald-50", text: "text-emerald-700", icon: <BoltIcon className="h-5 w-5" /> }
  ];
  
  const vitalsData = [
    { label: "Blood glucose", value: "120 mg/dt", icon: <BeakerIcon className="h-5 w-5" /> },
    { label: "Weight", value: "55 Kg", icon: <ScaleIcon className="h-5 w-5" /> },
    { label: "Heart rate", value: "70 bpm", icon: <HeartIcon className="h-5 w-5" /> },
    { label: "Temperature", value: "36.9 °C", icon: <FireIcon className="h-5 w-5" /> },
    { label: "Blood pressure", value: "120/80", icon: <BoltIcon className="h-5 w-5" /> }
  ];
    ...
    <div className="bg-white rounded-xl p-6 mb-6 shadow-sm hover:shadow-md">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {vitalsData.map((item, idx) => {
          ...
          return (
            <div key={idx} className={`${colorScheme.bg} rounded-lg p-4 border border-opacity-20 transition-transform hover:transform hover:scale-105`}>
              <div className="flex items-center justify-between mb-2">
                <p className={`text-xs font-medium ${colorScheme.text}`}>{item.label}</p>
                <div className={`${colorScheme.text}`}>
                  {item.icon}
                </div>
              </div>
              <p className={`font-semibold ${colorScheme.text} text-lg`}>{item.value}</p>
            </div>
          );
        })}
      </div>
...
const AppointmentHistory = ({ selectedDate, setSelectedDate }) => {
  // Sample appointment data
  const appointments = [
    { id: 1, date: '2025-05-17', time: '10:30 AM', doctor: 'Dr. Sarah Johnson', concern: 'Regular Checkup' },
    { id: 2, date: '2025-05-18', time: '02:15 PM', doctor: 'Dr. Michael Chen', concern: 'Headache and Dizziness' },
    { id: 3, date: '2025-05-19', time: '09:00 AM', doctor: 'Dr. Lisa Williams', concern: 'Neck Pain' },
    { id: 4, date: '2025-05-20', time: '11:00 AM', doctor: 'Dr. Robert Garcia', concern: 'Follow-up Consultation' }
  ];

  // Filter appointments by selected date
  const filteredAppointments = selectedDate ? appointments.filter(app => app.date === selectedDate) : appointments;
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm">
      ...
      {filteredAppointments.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {["Admitted Date", "Time", "Dr Assigned", "Concern"].map((head, idx) => (
        ...
        <div className="bg-gray-50 rounded-lg p-8 text-center">
          ...
          <button onClick={() => setSelectedDate("")} className="mt-4 text-sm font-medium text-bimec-green hover:text-bimec-heavy-green flex items-center gap-1 justify-center mx-auto">
...
const MedicalRecord = ({ selectedDate, setSelectedDate }) => {
  ...
  const medicalRecords = [
    {
      id: 1, date: '2025-05-19',
      medications: [
        { name: "Amoxicillin 500mg", dosage: "1 Pill", time: "08:00 AM, 08:00 PM", type: "Antibiotic", notes: "Take with food." },
        { name: "Ibuprofen 400mg", dosage: "1 Pill", time: "As needed", type: "Pain Relief", notes: "Take for fever or pain." }
      ],
      tests: [
        { name: "Complete Blood Count", time: "09:30 AM", result: "Normal Range", notes: "All parameters normal." }
      ]
    },
    {
      id: 2, date: '2025-05-20',
      medications: [
        { name: "Indever 20", dosage: "1 Pill", time: "02:20 PM", type: "Emergency", notes: "For blood pressure." }
      ],
      tests: [
        { name: "ECG Test", time: "01:45 PM", result: "Minor Abnormalities", notes: "Slight irregularity detected." }
      ]
    }
  ];

  // Filter records by selected date
  const filteredRecords = selectedDate ? medicalRecords.filter(record => record.date === selectedDate) : medicalRecords;
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm">
      ...
      {filteredRecords.length > 0 ? (
            ...
            <div key={record.id} className="border border-gray-100 rounded-lg overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-medium text-bimec-black">Medical Record</h4>
                ...
                <ChevronDownIcon className={`h-5 w-5 text-gray-500 transition-transform ${expandedRecords[record.id] ? 'transform rotate-180' : ''}`} />
              </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        {record.medications.map((med, idx) => (
                          <div key={idx} className="bg-white border border-gray-100 rounded-lg p-4 hover:shadow-sm">
                            <div className="flex items-start justify-between">
                              ...
                              <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${
                                ...
                                {med.type === 'Emergency' ? <BoltIcon className="h-3 w-3" /> : <AcademicCapIcon className="h-3 w-3" />}
                                {med.type}
                              </span>
                            </div>
                            {med.notes && (
                    ...
                    <div>
                      ...
                      <div className="space-y-4 mt-4">
                        {record.tests.map((test, idx) => (
                          <div key={idx} className="border border-gray-100 rounded-lg p-4 hover:shadow-sm">
        ...
        <div className="bg-gray-50 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-600">No medical records found for the selected date.</p>
          <button onClick={() => setSelectedDate("")} className="mt-4 text-sm font-medium text-bimec-green hover:text-bimec-heavy-green flex items-center gap-1 justify-center mx-auto">
            <GlobeAltIcon className="h-4 w-4" />
            View all records
          </button>
          ...
          <section className="bg-gray-50 flex-1 overflow-auto p-8">
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-500 mb-4">Please select a patient to view their details</p>
                <button onClick={() => window.history.back()} className="bg-bimec-green text-white px-4 py-2 rounded-lg font-medium hover:bg-bimec-heavy-green flex items-center gap-2 justify-center mx-auto">
      ...
      <main className="flex-1 flex flex-col bg-white">
        ...
        <section className="bg-gray-50 flex-1 overflow-auto p-4">
          ...
          <section>
            ...
            {activeTab === 'Appointment History' && <AppointmentHistory selectedDate={selectedDate} setSelectedDate={setSelectedDate} />}
            {activeTab === 'Medical Record' && <MedicalRecord selectedDate={selectedDate} setSelectedDate={setSelectedDate} />}
          </section>
        </section>
      </main>
    </div>
